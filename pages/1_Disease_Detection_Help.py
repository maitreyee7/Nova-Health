...
st.title("🧠 Disease Prediction & Assistance")
# Absolute paths
...
# Your helper() and get_predicted_value() functions go here
# or import from a helpers.py file

# (Place your symptoms_dict and diseases_list here)
...
def helper(dis):
    ...
    pre = [str(p) for p in pre if pd.notna(p)]

    # Medications
    med = medications[medications['Disease'] == dis]['Medication']
    med = med.values.flatten()
    med = [str(m) for m in med if pd.notna(m)]

    # Diets
    die = diets[diets['Disease'] == dis]['Diet']
    die = die.values.flatten()
    die = [str(d) for d in die if pd.notna(d)]

    # Workout
    wrkout = workout[workout['disease'] == dis]['workout']
    wrkout = wrkout.values.flatten()
    wrkout = [str(w) for w in wrkout if pd.notna(w)]
...
user_symptoms = st.multiselect("Start typing to search symptoms", options=all_symptoms)
